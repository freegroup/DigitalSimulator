package de.hagemann.digitalsimulator.plugin.NumericDisplay;
...
public class NumericDisplay extends ObjectPlugin implements ILiveUpdateObject
{
   ...
   static final int    Width       = 3*PointOffset +4*PointWidth +20;
   static final int    PinOffset   = -32;
   static final String DescName    = "OBJ_NumericDisplay_Description";
   static final String Description = "Ziffernanzeige (0..F) incl. Decoder";
   ...
   public String getKey()            { return "Numeric-Display"; }
   public String getName()           { return "OBJECT.NUMERICDISPLAY"; }
  ...
  public boolean doConfigure(byte[] param) { return false; }
   ...
   {
      int NewValue = 0;
      for (int i=0; i<4; i++) NewValue = NewValue | (input[i] << i);
   ...
   public void paint(int xPos, int yPos, byte[] param)
   {
      ...
      { 
         int x = xPos+10;
         byte Wert = HEXBitmuster[param[Value]][i];
         for (int j=3; j>-1; j--)
         {
