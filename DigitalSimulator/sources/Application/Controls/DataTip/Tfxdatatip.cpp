...
short TFXDataTip::_showDelay = 2000;
short TFXDataTip::_hideDelay = 4000;
...
void TFXDataTip::Set(CPoint point, const CString& tip)
{
    ...
    if (rect.PtInRect(point))
    {
        ...
        if (point != m_origin)
        {
            ...
            KillTimer(m_timerToShow);
            KillTimer(m_timerToHide);
            SetTimer(m_timerToShow, _showDelay, NULL);
        }
    }
}
...
* NAME:             Stop
*
* DESCRIPTION:      This method stopes all timers. This is neccessary for
*                   parent views which use drag drop features. The timer will
*                   be fetch the focus, and the dragdrop action is destroyed.
*
* PARAMETERS:       none
*
* RETURN TYPE:      void
...
void TFXDataTip::OnTimer(UINT nIDEvent) 
{
   ...
   if( nIDEvent == m_timerToHide) 
	...
	else if(nIDEvent == m_timerToShow)
	{
		KillTimer(m_timerToShow);
      SetTimer(m_timerToHide,_hideDelay,NULL);
