...
public  class ObjectFactory
{
    ...
    {
        // erst einmal in die bereits geladenen und freien objekte
        // nachschauen.
        //
        for(Iterator iter = freeObjects.iterator();iter.hasNext();)
        {
            ...
            {
                Trace.info("Object ["+eobj.getType()+"] in preInstantiationCache found: "+eobj);
                if(eobj.getIsSingelton()>0)
                {
                    // singelton => nur einmal hinzufuegen und NICHT aus dem cache löschen.
                ...
                else
                {
                    ...
                    // und der Liste der Verwendeten Objekte hinzufügen
                    usedObjects.add(eobj);
                }
                Trace.info("return eobj="+eobj);
                return eobj;
            }
        }

        Trace.info("Keine bereits angelegtes Objekt ["+type+"] im Cache gefunden. Hole neues aus der Datenbank.");
        ...
        if(eobj!=null)
        {
            ...
            if(eCases.size()>0)
            {
              Trace.info("Reserve resource for object :"+eobj);
              ElectricCase eCase =(ElectricCase) eCases.get(0);
                ...
                for(Iterator iter = dep.iterator(); iter.hasNext();)
                {
                    ...
                    while(depIter.hasNext())
                    {
                       ...
                       long objectPinNum = dp.getObjectPin2CasePin().getObjectPin().getNumber();
            ...
            {
              Trace.info("Reserve object for Macro object:"+eobj);
        ...
        new MsgBox(I18N.get("Objekt nicht gefunden: ")+type);
        return null;
    }
