...
public class CaseView extends JPanel implements GraphSelectionListener, KeyListener
{
  ...
  ElectricCase electricCase;
  
    public CaseView(ElectricCase _electricCase)
    {
      electricCase = _electricCase;
      try
      {
      	setLayout(new BorderLayout());
      	// Construct the Graph
      	graph = new CaseGraph(new Model(),this);
	...
	public void insert(ElectricCase eCase) 
	{	
		// Construct Vertex with no Label
    GUIElectricCase vertex = new GUIElectricCase(graph, eCase);
    // add all electric objects in this case
    //
    ArrayList o2c= eCase.getElectricObjects();
    Iterator iter = o2c.iterator();
    while (iter.hasNext())
		{
			ElectricObject element = (ElectricObject) iter.next();
			new GUIElectricObject(graph,element, eCase);
		}
 	}
  ...
  public void save()
  {
    try
    {
      ...
      for(int i=0; i<objs.length; i++)
      {
        ...
        if(objs[i] instanceof GUIElectricCase)
        {
          GUIElectricCase node = (GUIElectricCase)objs[i];
          node.save();
        }
